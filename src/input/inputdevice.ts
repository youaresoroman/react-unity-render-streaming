...
export class Mouse extends InputDevice {
   ...
   */
  queueEvent(event: MouseEvent|WheelEvent) {
...
export class Keyboard extends InputDevice {
   ...
   */
  queueEvent(event: KeyboardEvent) {
...
export class Touchscreen extends InputDevice {
   ...
   */
  queueEvent(event: TouchScreenEvent, time) {
...
export class TouchscreenState extends IInputState {
  ...
  static convertPhaseId(type: "touchstart" | "touchend" | "touchmove" | "touchcancel") {
   ...
   * @param {Date} time
   */
  constructor(event: TouchEvent, state: TouchscreenState, time: Date) {
    super();
...
export class GamepadState extends IInputState {
   ...
   *
   * field offset 4
   * @member {Array} leftStick;
   *
   * field offset 12
   * @member {Array} rightStick;
   *
   * field offset 20
...
export class TextEvent {
  ...
  static create(deviceId, event, time) {
    ...
    textEvent.baseEvent = new InputEvent(
      TextEvent.format,
      eventSize,
      deviceId,
      time
